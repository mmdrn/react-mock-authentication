...
const loginHandler = http.post(`${baseUrl}/api/signin`, async ({ request }) => {
    await delay(2000)
    ...
    const user = users.find(
        (i) => i.username === formData.get('username') && i.password === formData.get('password')
    )
...
const getAuthUserHandelr = http.get(`${baseUrl}/api/getAuthUser`, async ({ request }) => {
    const validation: any = {}
